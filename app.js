      ...
      .then((user) => {
        ...
        passport.authenticate("local", function (err, authenticatedUser, info) {
          ...
          if (!authenticatedUser) {
            return res
              .status(401)
              .render("login.ejs", { error: "Invalid user ID or Password" });
          }
      ...
      function (err, user) {
        if (err) {
          if (err.name === "UserExistsError") {
            ...
            return res
              .status(400)
              .send("Username already exists. Please choose another username.");
          }

          if (
            err.code === 11000 &&
            err.keyPattern &&
            err.keyPattern.registration
          ) {
...
app.get("/verify-email", async (req, res) => {
  ...
  try {
    ...
    res.render("login.ejs", {
      error: "Email verified successfully! You can now log in.",
    });
  } catch (err) {
    // console.error(err);
...
app.route("/attendence").get(function (req, res) {
  if (req.isAuthenticated()) {
...
app
  ...
  .post(function (req, res) {
    async function fetchData() {
      ...
      if (arr && arr.length > 0) {
        ...
        arr.forEach((i) => {
          ...
          complaintmodel.updateOne({ _id: i.userid }, update).then((result) => {
    ...
    if (v === "All") {
      complaintmodel
        .find({ hostel: req.user.hostel, username: { $ne: req.user.username } })
        .then((data) => {
          // console.log(data);
...
// -----------user_inforation---------------------------------------------
app.route("/information").get(function (req, res) {
  if (req.isAuthenticated()) {
...
app
  ...
  .get(function (req, res) {
      ...
      if (req.user.role === "admin") {
        ...
        complaintmodel
          ...
          .then((data) => {
            ...
            for (let i = 0; i < data.length; i++) {
              if (data[i].status === "open") opena++;
              else if (data[i].status === "close") closea++;
              else inprogressa++;
            }
    ...
    } else res.render("login.ejs", { error: "Login To View Adminprofile" });
  })
  .post(async function (req, res) {
    try {
      ...
      if (req.body.choose === "tick1") {
        ...
        if (opena < 0) opena = 0;
        inprogressa++;
        result = await complaintmodel.updateOne(
          { _id: req.body.c_id },
        ...
        status = "completed";
        inprogressa--;
        if (inprogressa < 0) inprogressa = 0;
        closea++;
        result = await complaintmodel.updateOne(
          { _id: req.body.c_id },
...
app
  ...
  .get(function (req, res) {
    if (req.isAuthenticated()) {
      ...
      } else res.send("Access Denied:This is Only for Admin!");
...
var isAdmin = 0;
app.route("/menu").get(async function (req, res) {
  if (req.isAuthenticated()) {
    try {
